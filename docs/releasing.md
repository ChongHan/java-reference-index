...
Choose a final release version, for example `0.1.8`. The Gradle Plugin Portal rejects `SNAPSHOT` versions, and published plugin versions cannot be overwritten.

Run the build and Plugin Portal validation:

```bash
./gradlew build -PreleaseVersion=0.1.8
./gradlew :reference-index-gradle-plugin:publishPlugins --validate-only -PreleaseVersion=0.1.8
```
...
```bash
./gradlew :reference-index-gradle-plugin:publishPlugins -PreleaseVersion=0.1.8
```
...
```bash
./gradlew publishAggregationToCentralPortal -PreleaseVersion=0.1.8
```
...
```bash
curl -I https://repo.maven.apache.org/maven2/io/github/chonghan/reference-index-gradle-plugin/0.1.8/reference-index-gradle-plugin-0.1.8.pom
curl -I https://repo.maven.apache.org/maven2/io/github/chonghan/java-reference-index/io.github.chonghan.java-reference-index.gradle.plugin/0.1.8/io.github.chonghan.java-reference-index.gradle.plugin-0.1.8.pom
```

Check the Plugin Portal page after publishing:

```text
https://plugins.gradle.org/plugin/io.github.chonghan.java-reference-index/0.1.8
```
